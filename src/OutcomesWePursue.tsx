...
export const OutcomesWePursuePage: React.FC<OutcomesWePursuePageProps> = ({ onNavigate }) => {
  ...
  return (
    <div style={{ fontFamily: "'Inter', sans-serif", backgroundColor: '#FFFFFF', color: '#0A1128' }}>
      ...
      {/* 2. DARK FILTER BAR SECTION ("MEASURED AGAINST EIGHT OUTCOMES") */}
      <section
        style={{
      ...
      >
        <div className="section-container">
          ...
          <div style={{ marginBottom: '36px' }}>
            ...
            >
              MEASURED AGAINST EIGHT OUTCOMES
            </div>
          ...
          {/* 8 Filter Pills Grid with Number Prefix Tags and Hover Enlargement */}
          <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap', maxWidth: '1200px' }}>
            {outcomesData.map((item) => {
              const isSelected = activeFilterPill === item.title;
              const isHovered = hoveredFilterPill === item.id;

              return (
                <button
                  key={item.id}
                  onClick={() => {
                    setActiveFilterPill(item.title);
                    scrollToOutcome(item.id);
                  }}
                  onMouseEnter={() => setHoveredFilterPill(item.id)}
                  onMouseLeave={() => setHoveredFilterPill(null)}
                  style={{
                    display: 'inline-flex',
                    alignItems: 'center',
                    gap: '10px',
                    ...
                    boxShadow: isHovered
                      ? '0 12px 28px rgba(82, 224, 203, 0.35)'
                      : 'none',
                    transition: 'all 0.3s cubic-bezier(0.16, 1, 0.3, 1)',
                    userSelect: 'none',
                  }}
                >
                  {/* Cyan Number Tag Prefix */}
                  <span
                    style={{
                      fontSize: '11.5px',
                      fontWeight: 900,
                      color: isSelected ? '#0A1128' : '#52E0CB',
